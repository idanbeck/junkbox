...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        if(t < self.nsteps):
            ratio = (float)(t) / float(self.nsteps)
            self.epsilon = self.eps_begin + ((float(self.eps_end) - float(self.eps_begin)) / float(self.nsteps)) * float(t)
        else:
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        roulette = np.random.random_sample()
        if(roulette <= self.epsilon):
