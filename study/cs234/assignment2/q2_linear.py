...
# Note: designed for tensorflow 1.7
...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        img_height, img_width, img_channels = state_shape

        self.s = tf.placeholder(tf.uint8, shape=[None, img_height, img_width, img_channels * self.config.state_history])
        self.a = tf.placeholder(tf.int32, shape=[None])
        self.r = tf.placeholder(tf.float32, shape=[None])
        self.sp = tf.placeholder(tf.uint8, shape=[None, img_height, img_width, img_channels * self.config.state_history])
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        #flat = tf.layers.flatten(state, scope=scope)

        flat = layers.flatten(state, scope=scope)
        out = layers.fully_connected(flat, num_actions, activation_fn=None, reuse=reuse, scope=scope)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        # Captures the values from the graph
        q_scope_collection = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        target_q_scope_collection = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)

        # sets up the operations to assign q_val to the target_q_vals
        operations = []
        for q_val, target_q_val in zip(q_scope_collection, target_q_scope_collection):
            operations.append(tf.assign(target_q_val, q_val))

        # creates the update target operation group
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_samp = self.r + self.config.gamma * tf.reduce_max(target_q, axis=1) * (1.0 - tf.cast(self.done_mask, tf.float32))
    ...
    def add_optimizer_op(self, scope):
        ...
        # compute grads with respect to variables in scope for self.loss
        vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope)
        gradients = adam.compute_gradients(self.loss, var_list=vars)

        # Clip gradients if grad clip set
        if self.config.grad_clip:
            gradients = [(tf.clip_by_norm(grad, self.config.clip_val), val) for grad,val in gradients if val is not None]

        self.train_op = adam.apply_gradients(gradients)

        gradients, _ = zip(*gradients)
